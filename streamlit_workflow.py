...
class StreamlitBankStatementWorkflow:
    ...
    async def process_stage1_with_progress(
        self,
        config: ProcessingConfig,
        progress_callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
        Process stage 1: PDF extraction through classification (steps 1-4)
        
        Returns:
            Dictionary with classification results and intermediate data for stage 2
        """
        try:
            ...
            processor = BankStatementProcessor(
                config.pdf_path, config.password)
            transactions = processor.process_all_pages(return_only=True)

            progress_callback(
                1, f"Extracted {len(transactions)} transactions from PDF")
            ...
            if config.force_reclassify:
                progress_callback(
                    2, "Force reclassify enabled - ignoring existing cache")
            ...
            total_cached_keywords = sum(len(type_cache)
                                        for type_cache in cache.values())
            progress_callback(
                2, f"Loaded {total_cached_keywords} cached keywords")
            ...
            if config.force_reclassify:
                ...
                print('identifying uncached transactions')
                uncached_transactions = self.workflow.identify_uncached_transactions(
                    transactions, cache)

            progress_callback(
                3, f"Found {len(uncached_transactions)} transactions needing classification")
            ...
            if uncached_transactions:
                progress_callback(
                    4, f"Classifying {len(uncached_transactions)} new transaction types...")
                ...
                progress_callback(
                    4, f"Classified {len(classification_results)} new transactions")
            ...
            # Return stage 1 data for user review
            stage1_data = {
                'transactions': transactions,
                'cache': cache,
                'uncached_transactions': uncached_transactions,
                ...
                'progress_info': {
                    ...
                    'cached_keywords': total_cached_keywords,
                    'uncached_transactions': len(uncached_transactions),
                    'new_classifications': len(classification_results) if not classification_results.empty else 0
                }
            }
            
            progress_callback(4, "Stage 1 completed - ready for user review")
            return stage1_data
    ...
    async def process_stage2_with_progress(
        self,
        config: ProcessingConfig,
        stage1_data: Dict[str, Any],
        edited_classifications: pd.DataFrame,
        progress_callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
        Process stage 2: Apply edited classifications and complete workflow (steps 5-10)
        
        Args:
            config: Processing configuration
            stage1_data: Data from stage 1 processing 
            edited_classifications: User-edited classification results
        ...
        try:
            ...
            transactions = stage1_data['transactions']
            cache = stage1_data['cache']
            
            # Step 5: Apply edited classifications to cache
            progress_callback(5, "Applying edited classifications to cache...")
            
            new_keywords = 0
            for _, row in edited_classifications.iterrows():
                keyword = row['keyword']
                ...
                if keyword and keyword not in cache[transaction_type]:
                    ...
                    # Update existing keyword with edited category
                    cache[transaction_type][keyword] = category

            progress_callback(5, f"Applied {len(edited_classifications)} classifications to cache")
            self.progress_tracker.add_step_info(5, {
                'edited_classifications_applied': len(edited_classifications),
                'new_keywords_added': new_keywords
            })

            # Step 6: Apply categories to all transactions
            progress_callback(6, "Applying categories to all transactions...")

            categorized_transactions = self.workflow.apply_categories_to_transactions(
                transactions, cache)
            ...
            progress_callback(
                6, f"Categorized: {categorized_count} transactions, Uncategorized: {uncategorized_count}")
            ...
            # Step 7: Clean descriptions using AI (optional)
            if config.clean_descriptions:
                progress_callback(
                    7, "Cleaning transaction descriptions using AI...")
                ...
                progress_callback(
                    7, f"Cleaned {len(unique_descriptions)} unique descriptions")
                ...
                progress_callback(
                    7, "Skipping description cleaning (disabled)")
                self.progress_tracker.add_step_info(
                    7, {'descriptions_cleaned': 0})

            # Step 8: Save final results
            progress_callback(
                8, f"Saving results to {os.path.basename(config.output_csv)}...")
            ...
            progress_callback(8, f"Saved {len(df)} categorized transactions")
            self.progress_tracker.add_step_info(8, {
                'output_file': os.path.basename(config.output_csv),
                'total_transactions_saved': len(df)
            })

            # Step 9: Generate Excel reports (optional)
            excel_report_path = None
            if config.generate_reports:
                progress_callback(9, "Generating Excel financial reports...")
                ...
                try:
                    ...
                    progress_callback(
                        9, f"Excel report generated with {len(available_months)} monthly sheets")
                    ...
                    progress_callback(9, f"Error generating reports: {str(e)}")
                    self.progress_tracker.add_step_info(9, {
                        'report_generation_error': str(e)
                    })
            else:
                progress_callback(
                    9, "Skipping Excel report generation (disabled)")
                self.progress_tracker.add_step_info(
                    9, {'excel_report_generated': False})

            # Step 10: Save updated cache and classification details
            progress_callback(10, "Finalizing processing results...")
            ...
            if config.save_classification_details and not edited_classifications.empty:
                classification_details_path = config.classification_details_file
                edited_classifications.to_csv(
                    classification_details_path, index=False)

            # Calculate category breakdown
            category_counts = df['category'].value_counts().to_dict()

            progress_callback(10, "Processing results finalized")
            self.progress_tracker.add_step_info(10, {
            ...
            results = {
                ...
                'cache_stats': {
                    ...
                    'cache_hit_rate': f"{((len(transactions) - len(stage1_data.get('uncached_transactions', []))) / len(transactions) * 100):.1f}%" if transactions else "0%"
                }
            }
        ...
        try:
            ...
            if uncached_transactions:
                ...
                classification_results = await self.workflow.classifier.classify_and_analyze(
                    ...
                    max_concurrent=config.max_concurrent,
                    return_only=True,
                    show_progress=True  # We'll handle progress ourselves
                )
                ...
                progress_callback(
                    4, f"Added {new_keywords} new keywords to cache")
                self.progress_tracker.add_step_info(4, {
                    'new_classifications': len(classification_results),
                    'new_keywords_added': new_keywords,
                    'classification_model': config.model
            ...
            progress_callback(
                5, f"Categorized: {categorized_count} transactions, Uncategorized: {uncategorized_count}")
            self.progress_tracker.add_step_info(5, {
                'categorized_transactions': categorized_count,
                'uncategorized_transactions': uncategorized_count,
                'categorization_rate': f"{(categorized_count / len(categorized_transactions) * 100):.1f}%" if categorized_transactions else "0%"
            ...
            if config.clean_descriptions:
                ...
                progress_callback(
                    6, f"Cleaned {len(unique_descriptions)} unique descriptions")
                self.progress_tracker.add_step_info(6, {
                    'descriptions_cleaned': len(unique_descriptions),
                    'cleaning_model': 'gpt-4.1-nano'
                })
                ...
                self.progress_tracker.add_step_info(
                    6, {'descriptions_cleaned': 0})

            # Step 7: Save final results
            progress_callback(
                7, f"Saving results to {os.path.basename(config.output_csv)}...")
            ...
            if config.generate_reports:
                ...
                try:
                    ...
                    progress_callback(
                        8, f"Excel report generated with {len(available_months)} monthly sheets")
                    self.progress_tracker.add_step_info(8, {
                        'excel_report': os.path.basename(excel_report_path),
                        'monthly_sheets': len(available_months),
                        'months_covered': ', '.join(month_names)
                ...
                progress_callback(
                    8, "Skipping Excel report generation (disabled)")
                self.progress_tracker.add_step_info(
                    8, {'excel_report_generated': False})
...
def run_streamlit_workflow(config: ProcessingConfig) -> Dict[str, Any]:
    ...
    try:
        results = asyncio.run(
            workflow.process_bank_statement_with_progress(config))
        return results
    except Exception as e:
